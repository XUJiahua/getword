...
- **UI Label** (15px, weight 650, line-height 1.4)：表单 label、按钮文字、section 标题。weight 650 不是 700：在中文里 700 太黑，650 在 PingFang SC 上仍能拉出层级感而不显粗野。
- **UI Body** (15px, weight 400, line-height 1.55)：input 输入文字、textarea、chip 内文字。
- **UI Hint** (12px, weight 400, line-height 1.45)：所有 `.hint` `.status` 提示文字。**不允许低于 12px**：家长老花会先在这里出问题。

> 之前 ui-label / ui-body 是 14px。15px 同时把字号阶梯收紧到 12 / 15 / 22（相邻比 1.25 / 1.47，过 DESIGN.md 的 1.25 门槛），并把表单文字抬到 PRODUCT.md 无障碍要求的"≥14px"以上一档，老花眼读得更稳。系统字栈上 15px 与 14px 视觉差异极小，但工程含义清晰多了：这一档不是"想加一点就加一点"，而是同时服务两个约束。
